<script>
    ...
    import ItemCard from './RefereeCard.svelte';
    ...
    const fetch = () => {
        promise = {};
        promise = apiFetch.get(`${COMPETITIONS_ENDPOINT}/referees/${$router.params.comp}/`, {withAuth: false})
            .then(result => {
        ...
        <div style="height: 100%">
            {#if result && result.data && result.data.length}
                <VirtualList items={(result.data || []).filter(({name=""}) => name.toLowerCase().indexOf(search.toLowerCase()) !== -1)} let:item>
                    <ItemCard uuid={item.referee_uuid}>
                            <div slot="controls"><Button on:click={e => {deleteAct(item.uuid).then(delay(100)).then(fetch)}} outline>Удалить</Button></div>       
                    </ItemCard>
                </VirtualList>
            {:else}
                Пока нет записей
